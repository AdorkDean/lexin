...
function InfoSub() {
    ...
    if (!InfoCheck_realname(realname)) {
        $('#realnametip').addClass('err_tip').find('span').html('真实姓名错误');
        return;
    }
    var realzip = $('#realzip').val();
    if (realzip.length < 1 || !IdCardValidate(realzip)) {
        $('#realziptip').addClass('err_tip').find('span').html('身份证号码错误');
        return;
    }
    var qq = $('#qq').val();
    if (qq.length > 0 && !isQQ(qq)) {
        $('#qqtip').addClass('err_tip').find('span').html('qq号码错误（可不填）');
        return;
    }
    var birth = $('#birth').val();
    var add = $('#address').val();
...
function BanEmail() {
    ...
    if (ban_email == '') {
        $('#banemailtip').addClass('err_tip').find('span').html('请输入邮箱地址。');
        return;
    } else if (!reg.test(ban_email)) {
        $('#banemailtip').addClass('err_tip').find('span').html('您输入的邮件地址不正确!');
        return;
    }
