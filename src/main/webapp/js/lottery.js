...
var Base = {
    ...
    Begin: function () {
        ...
        $('ul.betway_tabs li').click(function () {//选几星
            ...
            $('#palyer_con>.select_num>.way_tags div').eq(a_index).find("input").eq(0).attr("checked", true).click();
        });
        $('.way_tags input').click(function () {//选玩法
            Lot.Pay = $(this).val();
            Base.ClearBall();
        ...
        $('ul.fs_zx_btns>li').click(function () {//大小奇偶事件
            ...
            switch (txt) {
                case "全":
                    u.find('li').removeClass(Base.Ball).find('span').click();
                    break;
                case "大":
                    u.find('li').removeClass(Base.Ball).each(function (index) {
                        if (index > 4) {
                ...
                case "小":
                    ...
                    break;
                case "奇":
                    u.find('li').removeClass(Base.Ball);
                    u.find('li:' + odd).find('span').click();
                    break;
                case "偶":
                    u.find('li').removeClass(Base.Ball);
                    u.find('li:' + even).find('span').click();
                    break;
                case "清除":
                    u.find('li').removeClass(Base.Ball);
                    break;
        ...
        $('#confirm_select').click(function () {//选号添加到投注栏
            if ($(this).hasClass('addbtn')) {
                if (!$("#box_ds").is(":hidden")) {//单式
                    ...
                    var nm = PName[Lot.Pay];
                    var cd = Lot.Tcode;
                    var zs = Base.GetZhuShu(cd);
                    var h = '<dd style="background-color: transparent;"><span title="' + nm + cd + '[' + zs + '注 ' + zs * 2 + '元]" data="' + Lot.Pay + ':' + cd + ':' + zs + '" class="f_left"><b>' + nm + '&nbsp;&nbsp;<span class="c_ba2636">' + cd + '</span></b>&nbsp;[' + zs + '注 ' + zs * 2 + '元]</span><span class="f_right"><a href="javascript:;" title="删除" class="delete">删除</a></span></dd>';
        ...
        $('#del_ball_all').click(function () {
            /**清空认购栏*/
            $('#select_list').empty();
            Base.ShowMoney();
        });
        $('#touzhutype input').click(function () {
            /**投注方式*/
        ...
        $('#daigoutype input').click(function () {
            /**购买方式*/
        ...
        $('#get_data').click(function () {
            if (Lot.IsOpen != 0) {
                ...
                if (Lot.HMyMon < Math.ceil(Lot.AMoney * 0.05)) {
                    alert('合买发起人至少认购5%，' + Math.ceil(Lot.AMoney * 0.05) + '元。');
                    return;
    ...
    FormatDS: function () {//单式格式化
        ...
        var n = 0;
        var maxnum = 3;
        if (['361'].indexOf(Lot.Pay) >= 0) {
            maxnum = 8;
        } else if (['360'].indexOf(Lot.Pay) >= 0) {
            maxnum = 7;
        } else if (['359'].indexOf(Lot.Pay) >= 0) {
            maxnum = 6;
        } else if (['316', '358'].indexOf(Lot.Pay) >= 0) {
            maxnum = 5;
        } else if (['315', '357'].indexOf(Lot.Pay) >= 0) {
            maxnum = 4;
        } else if (['302', '355', '351', '351'].indexOf(Lot.Pay) >= 0) {
            maxnum = 2;
        } else if (['350'].indexOf(Lot.Pay) >= 0) {
            maxnum = 1;
        }
        for (var i = 0; i < len; i++) {
            if (i % maxnum == 0) {
    ...
    AddDSLine: function (dscode) {//单式添加
        ...
        for (var i = 0; i < codes.length; i++) {
            ...
            if (['322', '120'].indexOf(ptt) >= 0) {//组三单式
                ...
                if (codesp[0] == codesp[1]) {
                    eq += 1;
                }
                if (codesp[0] == codesp[2]) {
                    eq += 1;
                }
                if (codesp[2] == codesp[1]) {
                    eq += 1;
                }
                if (eq != 1) {
                    continue;
            ...
            if (Number(ptt) >= 350 && Number(ptt) <= 370) {
                if (!reg.test(codet) || distinct(codet)) {
                    continue;
                }
            }
            var nm = PName[ptt];
            var cd = qz + codet;
            var zs = Base.GetZhuShu(cd);
            var h = '<dd style="background-color: transparent;"><span title="' + nm + cd + '[' + zs + '注 ' + zs * 2 + '元]" data="' + ptt + ':' + cd + ':' + zs + '" class="f_left"><b>' + nm + '&nbsp;&nbsp;<span class="c_ba2636">' + cd + '</span></b>&nbsp;[' + zs + '注 ' + zs * 2 + '元]</span><span class="f_right"><a href="javascript:;" title="删除" class="delete">删除</a></span></dd>';
            $('#select_list').append(h);
        }
    },
    Buy: function () {//自购
    ...
    JHBT: function () {
        ...
        if (Lot.Zhushu * 2 > bonus) {//验证注数是否大于奖金
            alert("按照当前设置，投注金额" + (Lot.Zhushu * 2) + "元，超过收益" + bonus + "元，请重新设置！");
            return;
        }
    ...
    ShowPay: function (p) {
        ...
        } else if ('501' == p || '502' == p || '503' == p) {
            if ('501' == p) {
                $('#box_pk10_1').show();
            } else if ('502' == p) {
                $('#box_pk10_1,#box_pk10_2,#box_pk10_3').show();
            } else if ('503' == p) {
                $('#box_pk10_1,#box_pk10_2,#box_pk10_3,#box_pk10_4,#box_pk10_5,#box_pk10_6,#box_pk10_7,#box_pk10_8,#box_pk10_9,#box_pk10_10').show();
            }
        }
    },
    ...
    GetCode: function () {//获取选球
        ...
        } else if ('501' == l || '502' == l || '503' == l) {
            if ('501' == l) {
                $("#box_pk10_1").find("ul.fs_red_area").each(function (index) {
                    temp = "";
                    $(this).find('li.' + Base.Ball + ' span').each(function () {
                        temp += $(this).text()+" ";
                    })
                    re += temp.substr(0, temp.length-1) +",";
                });
            } else if ('502' == l) {
                $('#box_pk10_1,#box_pk10_2,#box_pk10_3').find("ul.fs_red_area").each(function (index) {
                    temp = "";
                    $(this).find('li.' + Base.Ball + ' span').each(function () {
                        temp += $(this).text()+" ";
                    })
                ...
                $('#box_pk10_1,#box_pk10_2,#box_pk10_3,#box_pk10_4,#box_pk10_5,#box_pk10_6,#box_pk10_7,#box_pk10_8,#box_pk10_9,#box_pk10_10').find("ul.fs_red_area").each(function (index) {
                    temp = $(this).find('li.' + Base.Ball + ' span').text();
                    if (temp.length <= 0) {
                        temp = '-';
                    }
    ...
    GetZhuShu: function (c) {//根据选求内容计算注数
        ...
        if ('320' == t || '116' == t) {//组三胆拖
            ...
            return zs;
        } else if ('501' == t || '502' == t || '503' == t) {
            zs = 0;
            if ('501' == t) {
                zs = (arr[0] == ""? 0 :arr[0].split(" ").length);
            }else if ('502' == t){
                zs = 1;
                for(var i=0;i<arr.length;i++) {
                    zs *= (arr[i] == "" ? 0 : arr[i].split(" ").length);
                }
            }
            return zs;
        } else if ('353' == t) {//前三
