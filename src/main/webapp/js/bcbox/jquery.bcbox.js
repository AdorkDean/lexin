(function ($) {
    ...
    $.fn.Drags = function (opts) {
        var ps = $.extend({
            zIndex: 20, opacity: .7, handler: null, onMove: function () {
            }, onDrop: function () {
            }, range: ''
        }, opts);
        var dragndrop = {
            drag: function (e) {
                ...
                if (typeof r == 'object') dragData.target.css({
                    left: dx < r[3] ? r[3] : (dx > r[1] ? r[1] : dx),
                    top: dy < r[0] ? r[0] : (dy > r[2] ? r[2] : dy)
                }); else if (r == '') dragData.target.css({left: dx, top: dy});
                dragData.target.css({'opacity': ps.opacity, 'cursor': 'move'});
                dragData.onMove(e);
            }, drop: function (e) {
                var dragData = e.data.dragData;
        ...
        return this.each(function () {
            var me = this;
            var handler = null;
            if (typeof ps.handler == 'undefined' || ps.handler == null) handler = $(me); else handler = (typeof ps.handler == 'string' ? $(ps.handler, this) : ps.handler);
            handler.bind('mousedown', {e: me}, function (s) {
                ...
                var newRange = [];
                if (ps.range == 'window') {
                    newRange = [target.css('position') == 'fixed' ? 0 : $(window).scrollTop(), target.css('position') == 'fixed' ? $(window).width() - target.outerWidth() : $(window).scrollLeft() + $(window).width() - target.outerWidth(), target.css('position') == 'fixed' ? $(window).height() - target.outerHeight() : $(window).scrollTop() + $(window).height() - target.outerHeight(), target.css('position') == 'fixed' ? 0 : $(window).scrollLeft()];
                } else {
                ...
                var dragData = {
                    ...
                    offLeft: target.css('position') == 'fixed' ? s.pageX - $(window).scrollLeft() : s.pageX,
                    offTop: target.css('position') == 'fixed' ? s.pageY - $(window).scrollLeft() : s.pageY,
                    oldCss: oldCss,
                    onMove: ps.onMove,
                    onDrop: ps.onDrop,
                    handler: handler,
